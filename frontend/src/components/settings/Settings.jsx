...
export default function Settings() {
  ...
  const [censorMode, setCensorMode] = useState("bleep");
    ...
    async function loadInitialSettings() {
      try {
        ...
        if (isMounted) {
          ...
          setAudioBufferSeconds(formatBufferValue(audioSettingsData.buffer_seconds ?? 0));
          setCensorMode(
            audioSettingsData.censor_mode === "mute" ? "mute" : "bleep"
          );
        }
      } catch (err) {
  ...
  async function handleRemoveWord(setId, word) {
    try {
      const res = await fetch(
        `${SETTINGS_URL}/filter-sets/${setId}/remove-word`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  async function handleCensorModeChange(value) {
    if (!value || value === censorMode) return;
    setCensorMode(value);
    try {
      const res = await fetch(`${SETTINGS_URL}/audio-processing`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ censor_mode: value }),
      });
      const data = await res.json();
      if (!res.ok) {
        throw new Error(data.detail || "Failed to save censor mode");
      }
      if (data.censor_mode) setCensorMode(data.censor_mode);
      toast.success("Settings saved");
    } catch (err) {
      console.error("Error updating censor mode:", err);
      toast.error("Failed to save censor mode");
    }
  }
  ...
  async function handleSaveBuffer() {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        throw new Error(data.detail || "Failed to save buffer");
      }

      setAudioBufferSeconds(formatBufferValue(data.buffer_seconds ?? parsedBuffer));
      toast.success("Settings saved");
    } catch (err) {
  ...
  const appearanceCard = (
    <Card className="shadow-none">
      <CardHeader>
        ...
        <CardDescription>
          Choose how ModSquad looks on your device.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <ToggleGroup
  ...
  const audioProcessingCard = (
    <Card className="shadow-none">
      <CardHeader>
        ...
        <CardDescription>
          Configure how flagged audio is censored
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="space-y-2">
          <div className="flex items-center gap-1.5">
            <p className="text-sm font-medium">Censor mode</p>
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  type="button"
                  aria-label="Censor mode info"
              ...
              <TooltipContent>
                Bleep replaces flagged audio with a tone. Mute silences it.
              </TooltipContent>
            </Tooltip>
          </div>
          <ToggleGroup
            type="single"
            value={censorMode}
            onValueChange={handleCensorModeChange}
            aria-label="Censor mode"
          >
            <ToggleGroupItem value="bleep">Bleep</ToggleGroupItem>
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-1.5">
            ...
            <Tooltip>
              ...
              <TooltipContent>
                Adds extra time before and after flagged words when
                muting/bleeping audio.
              </TooltipContent>
            </Tooltip>
          </div>
          <div className="flex gap-2">
            <Input
              ...
              onBlur={() => {
                if (audioBufferSeconds === "") return;
                setAudioBufferSeconds(formatBufferValue(audioBufferSeconds));
              }}
              onKeyDown={(e) => {
                if (e.key === "Enter") handleSaveBuffer();
              }}
              placeholder="0.0"
            />
            <Button onClick={handleSaveBuffer} disabled={isSavingAudio}>
              Save
            </Button>
          </div>
        </div>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <label className="flex items-center gap-2 text-sm">
              ...
              <Switch checked={censorWords} onCheckedChange={setCensorWords} />
            </label>
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="space-y-3">
            {filterSets.map((filterSet) => {
              ...
              const query = (searchTermsBySet[filterSet.id] || "")
                .trim()
                .toLowerCase();
